...
class CarpentryComplaintsListView extends ConsumerWidget {
  const CarpentryComplaintsListView({super.key});
...
class _AllComplaintsView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final allComplaints = ref.watch(
      capentryComplaintsListViewModelProvider.select((_) => _.allComplaints),
    );
    final status = ref.watch(
      capentryComplaintsListViewModelProvider.select((_) => _.status),
    );
    final errorMessage = ref.watch(
      capentryComplaintsListViewModelProvider.select((_) => _.error),
    );

    return status == CarpentryComplaintsListViewStatus.loading
        ? const Center(child: CircularProgressIndicator())
        : status == CarpentryComplaintsListViewStatus.error
            ? Center(
...
class _PendingComplaintsView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final pendingComplaints = ref.watch(
      capentryComplaintsListViewModelProvider
